...
export async function POST(request: NextRequest) {
  ...
  const lead = await prisma.lead.create({
    data: {
      ...
      addOnPainter: input.addOns.painterCount > 0,
      addOnPainterCount: input.addOns.painterCount,
      shadeCode: input.shadeCode ?? null,
      customShadeBrand: input.customShade?.brand || null,
      customShadeCode: input.customShade?.code || null,
      customShadeNote: input.customShade?.note || null,
