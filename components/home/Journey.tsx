    ...
    <section className="blob frame relative overflow-hidden px-4 pt-[46px] lg:px-[var(--pad)] lg:pt-[200px] [&::before]:right-[-70px] [&::before]:bottom-20 [&::before]:h-[300px] [&::before]:w-[210px] [&::before]:opacity-70 lg:[&::before]:right-0 lg:[&::before]:bottom-[180px] lg:[&::before]:h-[560px] lg:[&::before]:w-[430px] lg:[&::before]:opacity-50">
      ...
      <div className="hscroll relative -mx-4 px-4 pb-1.5 lg:mx-0 lg:grid lg:grid-cols-2 lg:gap-10 lg:overflow-visible lg:px-0">
        {steps.map((step, i) => (
          <article
            key={step.num}
            className={`flex w-[280px] flex-col rounded-[16px] p-6 lg:min-h-[300px] lg:w-auto lg:p-[46px] ${step.bg}`}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="display text-[34px] leading-none text-current lg:text-[46px]">
                  {step.num}
                </div>
                <div className="display mt-1 text-xl text-current lg:text-[26px]">
                  {step.label}
                </div>
              </div>
              <div className="shrink-0 lg:origin-top-right lg:scale-125">
                <StepIcon index={i} />
              </div>
            </div>
            <p className="mt-6 text-sm leading-[1.45] lg:mt-[34px] lg:text-[19px] lg:leading-[1.42]">
              {step.body}
            </p>
          </article>
        ))}
