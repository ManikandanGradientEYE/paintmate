...
export default function PaintSection() {
  const { state, dispatch, selectedPaint, availablePaints } = useQuote();

  return (
    <Card>
      <h2 className="text-base font-bold text-ink">Choose your paint</h2>
      ...
      {availablePaints.other.length > 0 && (
        <>
          ...
          <p className="mt-3 text-xs text-ink-faint">
            Prices for other brands are approx. MRP — indicative only. Final price
      ...
      {state.addOns.painter && (
        <p className="mt-2 text-xs text-ink-faint">
          Painter labour — quote confirmed on WhatsApp based on scope.
        </p>
      )}
